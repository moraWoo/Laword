...
class SettingsViewController: UIViewController, SettingsViewProtocol {
    ...
    override func viewDidLoad() {
        ...
        dataSource = UICollectionViewDiffableDataSource<HeaderItem, SFSymbolItem>(collectionView: collectionView) { [unowned self]
            ...
            switchInCell.tag = indexPath.item
            print("switchInCell tag=\(switchInCell.tag)")
            switchInCell.addTarget(self, action: #selector(switchChanged), for: .valueChanged)
    ...
    @objc func defaultsChanged(){
        if UserDefaults.standard.bool(forKey: "dark_mode") {
            ...
            print("UserDefaults dark_mode222: \(val.description)")
        }
    }
}
