...
class MainPresenter: MainViewPresenterProtocol {
    ...
    required init(view: MainViewProtocol, dataStoreManager: DataStoreManagerProtocol) {
        ...
        fetchedWords = dataStoreManager.getWords(
            showKey: false,
            currentDateTime: dateTime,
            dictionaryName: dictionaryName ?? "5000 Oxford Words"
        )
    ...
    func getDataFromFile(nameOfFileDictionary: String, nameOfDictionary: String) {
        dataStoreManager.getDataFromFile(
            nameOfFileDictionary: nameOfFileDictionary,
            nameOfDictionary: nameOfDictionary
        )
    }
    ...
    func saveKeys(
        ...
        wordShowNow: String,
        grade: Grade) {
        dataStoreManager.saveKeys(
            word: word,
            key: key,
            wordTranslation: wordTranslation,
            wordShowed: wordShowed,
            wordShowNow: wordShowNow,
            grade: grade
        )
    }
